...
-- 補足：
-- ・すでに存在する場合にエラーを防ぐには「IF NOT EXISTS」を付ける
CREATE DATABASE IF NOT EXISTS bookstore;

-- 作成後は USE で使用するデータベースを指定
USE bookstore;
...
-- 補足：
-- ・DELETE は条件を付けないと全削除になるため注意
-- ・外部キー制約があると削除できないことがある
-- ・大量削除時はトランザクション管理が推奨される（BEGIN / COMMIT）
